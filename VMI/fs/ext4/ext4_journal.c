...
int ext4fs_init_journal(void)
{
    ...
    {
        journal_ptr[i] = malloc(sizeof(struct journal_log));
        if (!journal_ptr[i])
            goto fail;
        dirty_block_ptr[i] = malloc(sizeof(struct dirty_blocks));
    ...
    if (fs->blksz == 4096)
    {
        temp = malloc(fs->blksz);
        if (!temp)
            goto fail;
        journal_ptr[gindex]->buf = malloc(fs->blksz);
...
void ext4fs_dump_metadata(void)
{
    ...
    for (i = 0; i < MAX_JOURNAL_ENTRIES; i++)
    {
        if (dirty_block_ptr[i]->blknr == -1)
            break;
        put_ext4((uint64_t) ((uint64_t)dirty_block_ptr[i]->blknr * (uint64_t)fs->blksz), dirty_block_ptr[i]->buf, fs->blksz);
    }
}
...
int ext4fs_put_metadata(char *metadata_buffer, long int blknr)
{
    ...
    dirty_block_ptr[gd_index]->buf = malloc(fs->blksz);
...
static struct revoke_blk_list *_get_node(void)
{
    ...
    tmp_node = malloc(sizeof(struct revoke_blk_list));
    if (tmp_node == NULL)
        return NULL;
...
void recover_transaction(int prev_desc_logical_no)
{
    ...
    char *metadata_buff = malloc(fs->blksz);
    if (!temp_buff || !metadata_buff)
        goto fail;
    i = prev_desc_logical_no;
    ext4fs_read_inode(ext4fs_root, EXT2_JOURNAL_INO, (struct ext2_inode *)&inode_journal);
    blknr = read_allocated_block((struct ext2_inode *)&inode_journal, i);
    ...
    do
    {
        ...
        {
            if (check_blknr_for_revoke(be32_to_cpu(tag->block),be32_to_cpu(jdb->h_sequence)) == 0)
                continue;
        }
        blknr = read_allocated_block(&inode_journal, i);
        ext4fs_devread((lbaint_t)blknr * fs->sect_perblk, 0, fs->blksz, metadata_buff);
        put_ext4((uint64_t)((uint64_t)be32_to_cpu(tag->block) * (uint64_t)fs->blksz), metadata_buff, (uint32_t) fs->blksz);
    }
...
int ext4fs_check_journal_state(int recovery_flag)
{
    ...
    temp_buff = malloc(fs->blksz);
    if (!temp_buff)
        return -ENOMEM;
    temp_buff1 = malloc(fs->blksz);
    if (!temp_buff1)
    {
        free(temp_buff);
        return -ENOMEM;
    ...
    if (!(jsb->s_feature_compat & cpu_to_be32(JBD2_FEATURE_COMPAT_CHECKSUM)))
        jsb->s_feature_compat |= cpu_to_be32(JBD2_FEATURE_COMPAT_CHECKSUM);
    ...
    while (1)
    {
        ...
        ext4fs_devread((lbaint_t)blknr * fs->sect_perblk, 0, fs->blksz, temp_buff1);
        jdb = (struct journal_header_t *) temp_buff1;

        if (be32_to_cpu(jdb->h_blocktype) == EXT3_JOURNAL_DESCRIPTOR_BLOCK)
        {
            if (be32_to_cpu(jdb->h_sequence) != be32_to_cpu(jsb->s_sequence))
            {
        ...
        else if (be32_to_cpu(jdb->h_blocktype) == EXT3_JOURNAL_COMMIT_BLOCK)
        {
            if (be32_to_cpu(jdb->h_sequence) != be32_to_cpu(jsb->s_sequence))
            {
                print_jrnl_status(recovery_flag);
            ...
            if (transaction_state == TRANSACTION_RUNNING || (DB_FOUND == NO))
            {
        ...
        else if (be32_to_cpu(jdb->h_blocktype) == EXT3_JOURNAL_REVOKE_BLOCK)
        {
            if (be32_to_cpu(jdb->h_sequence) != be32_to_cpu(jsb->s_sequence))
            {
                print_jrnl_status(recovery_flag);
    ...
    if (recovery_flag == RECOVER)
    {
        ...
        blknr = read_allocated_block(&inode_journal, EXT2_JOURNAL_SUPERBLOCK);
        put_ext4((uint64_t) ((uint64_t)blknr * (uint64_t)fs->blksz), (struct journal_superblock_t *)temp_buff, (uint32_t) fs->blksz);
...
static void update_descriptor_block(long int blknr)
{
    ...
    ext4fs_read_inode(ext4fs_root, EXT2_JOURNAL_INO, &inode_journal);
    jsb_blknr = read_allocated_block(&inode_journal, EXT2_JOURNAL_SUPERBLOCK);
...
void ext4fs_update_journal(void)
{
    ...
    for (i = 0; i < MAX_JOURNAL_ENTRIES; i++)
    {
        if (journal_ptr[i]->blknr == -1)
            break;
        blknr = read_allocated_block(&inode_journal, jrnl_blk_idx++);
        put_ext4((uint64_t) ((uint64_t)blknr * (uint64_t)fs->blksz), journal_ptr[i]->buf, fs->blksz);
    }
    blknr = read_allocated_block(&inode_journal, jrnl_blk_idx++);
