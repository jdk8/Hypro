...
static void ext4fs_update(void)
{
    ...
    /* update  super block */
    put_ext4((uint64_t)(SUPERBLOCK_SIZE), (struct ext2_sblock *)fs->sb, (uint32_t)SUPERBLOCK_SIZE);

    /* update block groups */
    for (i = 0; i < fs->no_blkgrp; i++)
    {
        fs->bgd[i].bg_checksum = ext4fs_checksum_update(i);
        put_ext4((uint64_t)((uint64_t)fs->bgd[i].block_id * (uint64_t)fs->blksz), fs->blk_bmaps[i], fs->blksz);
    }

    /* update inode table groups */
    for (i = 0; i < fs->no_blkgrp; i++)
    {
        put_ext4((uint64_t) ((uint64_t)fs->bgd[i].inode_id * (uint64_t)fs->blksz), fs->inode_bmaps[i], fs->blksz);
    }

    /* update the block group descriptor table */
    put_ext4((uint64_t)((uint64_t)fs->gdtable_blkno * (uint64_t)fs->blksz), (struct ext2_block_group *)fs->gdtable, (fs->blksz * fs->no_blk_pergdt));
...
int ext4fs_get_bgdtable(void)
{
    ...
    fs->gdtable = malloc(fs->blksz * fs->no_blk_pergdt);
    if (!fs->gdtable)
        return -ENOMEM;
    /* read the group descriptor table */
    status = ext4fs_devread((lbaint_t)fs->gdtable_blkno * fs->sect_perblk, 0, fs->blksz * fs->no_blk_pergdt, fs->gdtable);
    if (status == 0)
        goto fail;
...
static void delete_double_indirect_block(struct ext2_inode *inode)
{
    ...
    if (inode->b.blocks.double_indir_block != 0)
    {
        ...
        status = ext4fs_devread((lbaint_t)blknr * fs->sect_perblk, 0, fs->blksz, (char *)di_buffer);
        for (i = 0; i < fs->blksz / sizeof(int); i++)
        {
            if (*di_buffer == 0)
                break;

            printf("DICB releasing %u\n", *di_buffer);
            ...
            ext4fs_reset_block_bmap(*di_buffer, fs->blk_bmaps[bg_idx], bg_idx);
            di_buffer++;
            bgd[bg_idx].free_blocks++;
            fs->sb->free_blocks++;
            /* journal backup */
...
static void delete_triple_indirect_block(struct ext2_inode *inode)
{
    ...
    if (inode->b.blocks.triple_indir_block != 0)
    {
        ...
        status = ext4fs_devread((lbaint_t)blknr * fs->sect_perblk, 0, fs->blksz, (char *)tigp_buffer);
        for (i = 0; i < fs->blksz / sizeof(int); i++)
        {
            if (*tigp_buffer == 0)
                break;
            printf("tigp buffer releasing %u\n", *tigp_buffer);

            tip_buffer = malloc(fs->blksz);
            if (!tip_buffer)
                goto fail;
            tipb_start_addr = (unsigned int *)tip_buffer;
            status = ext4fs_devread((lbaint_t)(*tigp_buffer) * fs->sect_perblk, 0, fs->blksz, (char *)tip_buffer);
            for (j = 0; j < fs->blksz / sizeof(int); j++)
            {
                ...
                ext4fs_reset_block_bmap(*tip_buffer, fs->blk_bmaps[bg_idx], bg_idx);
            ...
            ext4fs_reset_block_bmap(*tigp_buffer, fs->blk_bmaps[bg_idx], bg_idx);
        ...
        printf("tigp buffer itself releasing %ld\n", blknr);
    }
fail:
    free(tib_start_addr);
    free(tipb_start_addr);
...
static int ext4fs_delete_file(int inodeno)
{
    ...
    {
        struct ext2fs_node *node_inode = malloc(sizeof(struct ext2fs_node));
        if (!node_inode)
            goto fail;
        ...
        for (i = 0; i < no_blocks; i++)
        {
            ...
            ext4fs_reset_block_bmap(blknr, fs->blk_bmaps[bg_idx], bg_idx);
            printf("EXT4_EXTENTS Block releasing %ld: %d\n", blknr, bg_idx);
    ...
    /* get the block no */
    inodeno--;
    blkno = le32_to_uint32(&(bgd[ibmap_idx].inode_table_id)) + (inodeno % le32_to_uint32(&(inode_per_grp))) / inodes_per_block;

    /* get the offset of the inode */
    blkoff = ((inodeno) % inodes_per_block) * fs->inodesz;

    /* read the block no containing the inode */
    read_buffer = malloc(fs->blksz);
    if (!read_buffer)
        goto fail;
    start_block_address = read_buffer;
    status = ext4fs_devread((lbaint_t)blkno * fs->sect_perblk, 0, fs->blksz, read_buffer);
    if (status == 0)
        goto fail;
...
int ext4fs_init(void)
{
    ...
    fs->no_blkgrp = (uint32_t)ext4fs_div_roundup((ext4fs_root->sblock.total_blocks - ext4fs_root->sblock.first_data_block), ext4fs_root->sblock.blocks_per_group);
    ...
    fs->blk_bmaps = malloc(fs->no_blkgrp * sizeof(char *));
    if (!fs->blk_bmaps)
        goto fail;
    for (i = 0; i < fs->no_blkgrp; i++)
    {
        fs->blk_bmaps[i] = malloc(fs->blksz);
        if (!fs->blk_bmaps[i])
            goto fail;
    }
    ...
    for (i = 0; i < fs->no_blkgrp; i++)
    {
        status = ext4fs_devread((lbaint_t)fs->bgd[i].block_id *fs->sect_perblk, 0, fs->blksz, (char *)fs->blk_bmaps[i]);
        if (status == 0)
            goto fail;
    }

    /* load all the available inode bitmap of the partition */
    fs->inode_bmaps = malloc(fs->no_blkgrp * sizeof(unsigned char *));
    if (!fs->inode_bmaps)
        goto fail;
    for (i = 0; i < fs->no_blkgrp; i++)
    {
        fs->inode_bmaps[i] = malloc(fs->blksz);
        if (!fs->inode_bmaps[i])
            goto fail;
    }
    ...
    for (i = 0; i < fs->no_blkgrp; i++)
    {
        status = ext4fs_devread((lbaint_t)fs->bgd[i].inode_id * fs->sect_perblk, 0, fs->blksz, (char *)fs->inode_bmaps[i]);
        if (status == 0)
            goto fail;
    }
...
void ext4fs_deinit(void)
{
    ...
    /* free journal */
    char *temp_buff = malloc(fs->blksz);
    if (temp_buff)
    {
        ...
        put_ext4((uint64_t) ((uint64_t)blknr * (uint64_t)fs->blksz), (struct journal_superblock_t *)temp_buff, fs->blksz);
        free(temp_buff);
    }
    ext4fs_free_journal();
			     ...
			     int pos, unsigned int len, char *buf)
{
    int i;
    int blockcnt;
    unsigned int filesize = le32_to_uint32(&(file_inode->size));
    struct ext_filesystem *fs = get_fs();
    ...
    for (i = pos / fs->blksz; i < blockcnt; i++)
    {
        ...
        if (blknr)
        {
            if (previous_block_number != -1)
            {
                ...
                {	/* spill */
                    put_ext4((uint64_t)((uint64_t)delayed_start << log2blksz), delayed_buf, (uint32_t) delayed_extent);
                    previous_block_number = blknr;
                    delayed_start = blknr;
                    delayed_extent = blockend;
                    delayed_buf = buf;
                    delayed_next = blknr + (blockend >> log2blksz);
                }
            }
            else
            {
					...
					unsigned long sizebytes)
{
    ...
    g_parent_inode = malloc(sizeof(struct ext2_inode));
    if (!g_parent_inode)
        goto fail;
    ...
    if (div64_32(bytes_reqd_for_file, fs->blksz) != 0)
    {
        blks_reqd_for_file++;
        printf("total bytes for a file %u\n", blks_reqd_for_file);
    }
    ...
    /* prepare file inode */
    inode_buffer = malloc(fs->inodesz);
    if (!inode_buffer)
        goto fail;
    ...
    ext4fs_allocate_blocks(file_inode, blocks_remaining, &blks_reqd_for_file);
    file_inode->blockcnt = (blks_reqd_for_file * fs->blksz) >> fs->dev_desc->log2blksz;

    temp_ptr = malloc(fs->blksz);
    if (!temp_ptr)
        goto fail;
    ibmap_idx = inodeno / ext4fs_root->sblock.inodes_per_group;
    inodeno--;
    itable_blkno = le32_to_uint32(&(fs->bgd[ibmap_idx].inode_table_id)) + (inodeno % le32_to_uint32(&(sblock->inodes_per_group))) / inodes_per_block;
    blkoff = (inodeno % inodes_per_block) * fs->inodesz;
    ...
    parent_inodeno--;
    parent_itable_blkno = le32_to_uint32(&(fs->bgd[ibmap_idx].inode_table_id)) + (parent_inodeno % le32_to_uint32(&(sblock->inodes_per_group))) / inodes_per_block;
    blkoff = (parent_inodeno % inodes_per_block) * fs->inodesz;
    if (parent_itable_blkno != itable_blkno)
    {
        memset(temp_ptr, '\0', fs->blksz);
        ext4fs_devread((lbaint_t)parent_itable_blkno * fs->sect_perblk, 0, fs->blksz, temp_ptr);
