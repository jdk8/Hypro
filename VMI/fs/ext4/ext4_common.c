...
void put_ext4(uint64_t off, void *buf, uint32_t size)
{
    ...
    unsigned char sec_buf[DEFAULT_SECTOR_SIZE];
    ...
    if ((startblock + (size >> log2blksz)) > (part_offset + fs->total_sect))
    {
    ...
    if (remainder)
    {
        ...
        {
            fs->dev_desc->block_read(fs->dev_desc, startblock, 1, sec_buf);
            temp_ptr = sec_buf;
            memcpy((temp_ptr + remainder), (unsigned char *)buf, size);
            fs->dev_desc->block_write(fs->dev_desc, startblock, 1, sec_buf);
        }
    }
    else
    {
        if (size >> log2blksz != 0)
        {
            fs->dev_desc->block_write(fs->dev_desc, startblock, size >> log2blksz, (unsigned long *)buf);
        }
        else
        {
            fs->dev_desc->block_read(fs->dev_desc, startblock, 1, sec_buf);
            ...
            fs->dev_desc->block_write(fs->dev_desc, startblock, 1, (unsigned long *)sec_buf);
        }
    }
}
...
int ext4fs_checksum_update(unsigned int i)
{
    ...
    if (fs->sb->feature_ro_compat & EXT4_FEATURE_RO_COMPAT_GDT_CSUM)
    {
        ...
        crc = ext2fs_crc16(~0, fs->sb->unique_id, sizeof(fs->sb->unique_id));
        crc = ext2fs_crc16(crc, &i, sizeof(i));
        crc = ext2fs_crc16(crc, desc, offset);
        offset += sizeof(desc->bg_checksum);	/* skip checksum */
        if(offset != sizeof(*desc))
        {
            printf("ext4fs_checksum_update offset fail!\n");
        }
    }
...
void ext4fs_update_parent_dentry(char *filename, int *p_ino, int file_type)
{
    ...
    zero_buffer = malloc(fs->blksz);
    if (!zero_buffer)
    {
        printf("No Memory\n");
        return;
    }
    root_first_block_buffer = malloc(fs->blksz);
...
restart:
    ...
    {
        root_blknr = read_allocated_block(g_parent_inode, direct_blk_idx);
        if (root_blknr == 0)
        {
    ...
    status = ext4fs_devread((lbaint_t)first_block_no_of_root* fs->sect_perblk, 0, fs->blksz, root_first_block_buffer);
    if (status == 0)
        goto fail;
    ...
    while (dir->direntlen > 0)
    {
        ...
        if (fs->blksz - totalbytes == dir->direntlen)
        {
            ...
            if ((fs->blksz - totalbytes - last_entry_dirlen) < new_entry_byte_reqd)
            {
                ...
                g_parent_inode->b.blocks.dir_blocks[direct_blk_idx] = ext4fs_get_new_blk_no();
                if (g_parent_inode->b.blocks.dir_blocks[direct_blk_idx] == -1)
                {
                    printf("no block left to assign\n");
                    goto fail;
                }
                put_ext4(((uint64_t)((uint64_t)g_parent_inode->b.blocks.dir_blocks[direct_blk_idx] * (uint64_t)fs->blksz)), zero_buffer, fs->blksz);
                g_parent_inode->size = g_parent_inode->size + fs->blksz;
                g_parent_inode->blockcnt = g_parent_inode->blockcnt + fs->sect_perblk;
                if (ext4fs_put_metadata(root_first_block_buffer, first_block_no_of_root))
                    goto fail;
                goto restart;
            }
...
static int search_dir(struct ext2_inode *parent_inode, char *dirname)
{
    ...
    for (direct_blk_idx = 0; direct_blk_idx < INDIRECT_BLOCKS; direct_blk_idx++)
    {
        ...
        block_buffer = malloc(fs->blksz);
        if (!block_buffer)
            goto fail;

        status = ext4fs_devread((lbaint_t)blknr * fs->sect_perblk, 0, fs->blksz, (char *)block_buffer);
        if (status == 0)
            goto fail;
    ...
    for (i = 1; i < depth; i++)
    {
        ...
        else
        {
            if (ptr[i + 1] != NULL)
            {
                ...
                if (ext4fs_iget(matched_inode_no, parent_inode))
                {
                    result_inode_no = -1;
                    goto fail;
                }
...
static int check_filename(char *filename, unsigned int blknr)
{
    ...
    if (found == 1)
    {
        if (ext4fs_put_metadata(root_first_block_addr, first_block_no_of_root))
            goto fail;
        ret = inodeno;
    }
...
long int ext4fs_get_new_blk_no(void)
{
    ...
    struct ext_filesystem *fs = get_fs();
    char *journal_buffer = malloc(fs->blksz);
    char *zero_buffer = malloc(fs->blksz);
    ...
    if (fs->first_pass_bbmap == 0)
    {
        for (i = 0; i < fs->no_blkgrp; i++)
        {
            if (bgd[i].free_blocks)
            {
                ...
                {
                    put_ext4(((uint64_t) ((uint64_t)bgd[i].block_id * (uint64_t)fs->blksz)), zero_buffer, fs->blksz);
                    bgd[i].bg_flags = bgd[i].bg_flags & ~EXT4_BG_BLOCK_UNINIT;
                    memcpy(fs->blk_bmaps[i], zero_buffer, fs->blksz);
                }
                fs->curr_blkno = _get_new_blk_no(fs->blk_bmaps[i]);
                if (fs->curr_blkno == -1)
                    /* if block bitmap is completely fill */
                    continue;
                fs->curr_blkno = fs->curr_blkno + (i * fs->blksz * 8);
                fs->first_pass_bbmap++;
                bgd[i].free_blocks--;
                fs->sb->free_blocks--;
                status = ext4fs_devread((lbaint_t)bgd[i].block_id * fs->sect_perblk, 0, fs->blksz, journal_buffer);
                if (status == 0)
                    goto fail;
                if (ext4fs_log_journal(journal_buffer, bgd[i].block_id))
                    goto fail;
                goto success;
            }
            else
...
restart:
        ...
        if (bgd[bg_idx].bg_flags & EXT4_BG_BLOCK_UNINIT)
        {
            ...
            put_ext4(((uint64_t) ((uint64_t)bgd[bg_idx].block_id * (uint64_t)fs->blksz)), zero_buffer, fs->blksz);
            memcpy(fs->blk_bmaps[bg_idx], zero_buffer, fs->blksz);
            bgd[bg_idx].bg_flags = bgd[bg_idx].bg_flags & ~EXT4_BG_BLOCK_UNINIT;
        }

        if (ext4fs_set_block_bmap(fs->curr_blkno, fs->blk_bmaps[bg_idx], bg_idx) != 0)
        {
        ...
        if (prev_bg_bitmap_index != bg_idx)
        {
            ...
            status = ext4fs_devread((lbaint_t)bgd[bg_idx].block_id* fs->sect_perblk, 0, fs->blksz, journal_buffer);
            if (status == 0)
                goto fail;
            if (ext4fs_log_journal(journal_buffer, bgd[bg_idx].block_id))
                goto fail;
...
int ext4fs_get_new_inode_no(void)
{
    ...
    if (fs->first_pass_ibmap == 0)
    {
        for (i = 0; i < fs->no_blkgrp; i++)
        {
            ...
            {
                if (bgd[i].bg_itable_unused != bgd[i].free_inodes)
                        bgd[i].bg_itable_unused = bgd[i].free_inodes;
                if (bgd[i].bg_flags & EXT4_BG_INODE_UNINIT)
                {
                    put_ext4(((uint64_t)((uint64_t)bgd[i].inode_id * (uint64_t)fs->blksz)), zero_buffer, fs->blksz);
                    bgd[i].bg_flags = bgd[i].bg_flags & ~EXT4_BG_INODE_UNINIT;
                    memcpy(fs->inode_bmaps[i], zero_buffer, fs->blksz);
                }
                fs->curr_inode_no = _get_new_inode_no(fs->inode_bmaps[i]);
                if (fs->curr_inode_no == -1)
                    /* if block bitmap is completely fill */
                    continue;
                fs->curr_inode_no = fs->curr_inode_no + (i * inodes_per_grp);
                fs->first_pass_ibmap++;
                bgd[i].free_inodes--;
                bgd[i].bg_itable_unused--;
                fs->sb->free_inodes--;
                status = ext4fs_devread((lbaint_t)bgd[i].inode_id * fs->sect_perblk, 0, fs->blksz, journal_buffer);
                if (status == 0)
                    goto fail;
                if (ext4fs_log_journal(journal_buffer, bgd[i].inode_id))
                    goto fail;
                goto success;
            } else
...
restart:
        ...
        {
            memset(zero_buffer, '\0', fs->blksz);
            put_ext4(((uint64_t) ((uint64_t)bgd[ibmap_idx].inode_id *(uint64_t)fs->blksz)), zero_buffer, fs->blksz);
            bgd[ibmap_idx].bg_flags = bgd[ibmap_idx].bg_flags & ~EXT4_BG_INODE_UNINIT;
            memcpy(fs->inode_bmaps[ibmap_idx], zero_buffer, fs->blksz);
        }

        if (ext4fs_set_inode_bmap(fs->curr_inode_no, fs->inode_bmaps[ibmap_idx], ibmap_idx) != 0)
        {
        ...
        {
            memset(journal_buffer, '\0', fs->blksz);
            status = ext4fs_devread((lbaint_t)bgd[ibmap_idx].inode_id* fs->sect_perblk, 0, fs->blksz, journal_buffer);
            if (status == 0)
                goto fail;
        ...
        if (bgd[ibmap_idx].bg_itable_unused != bgd[ibmap_idx].free_inodes)
					...
					unsigned int *no_blks_reqd)
{
    ...
    if (*total_remaining_blocks != 0)
    {
        ...
        status = ext4fs_devread((lbaint_t)si_blockno * fs->sect_perblk, 0, fs->blksz, (char *)si_buffer);
        ...
        put_ext4(((uint64_t) ((uint64_t)si_blockno * (uint64_t)fs->blksz)), si_start_addr, fs->blksz);
					...
					unsigned int *no_blks_reqd)
{
    ...
    if (*total_remaining_blocks != 0)
    {
        ...
        di_parent_buffer = malloc(fs->blksz);
        if (!di_parent_buffer)
            goto fail;

        di_block_start_addr = di_parent_buffer;
        (*no_blks_reqd)++;
        printf("DIPB %ld: %u\n", di_blockno_parent, *total_remaining_blocks);

        status = ext4fs_devread((lbaint_t)di_blockno_parent *fs->sect_perblk, 0, fs->blksz, (char *)di_parent_buffer);
        ...
        for (i = 0; i < (fs->blksz / sizeof(int)); i++)
        {
            ...
            *di_parent_buffer = di_blockno_child;
            di_parent_buffer++;
            (*no_blks_reqd)++;
            printf("DICB %ld: %u\n", di_blockno_child, *total_remaining_blocks);

            status = ext4fs_devread((lbaint_t)di_blockno_child * fs->sect_perblk, 0, fs->blksz, (char *)di_child_buff);
            ...
            put_ext4(((uint64_t) ((uint64_t)di_blockno_child * (uint64_t)fs->blksz)), di_child_buff_start, fs->blksz);
        ...
        put_ext4(((uint64_t) ((uint64_t)di_blockno_parent * (uint64_t)fs->blksz)), di_block_start_addr, fs->blksz);
					...
					unsigned int *no_blks_reqd)
{
    ...
    if (*total_remaining_blocks != 0)
    {
        ...
        ti_gp_buff = malloc(fs->blksz);
        if (!ti_gp_buff)
            return;

        ti_gp_buff_start_addr = ti_gp_buff;
        (*no_blks_reqd)++;
        printf("TIGPB %ld: %u\n", ti_gp_blockno, *total_remaining_blocks);
        ...
        for (i = 0; i < (fs->blksz / sizeof(int)); i++)
        {
            ...
            ti_parent_buff = malloc(fs->blksz);
            if (!ti_parent_buff)
                goto fail;
            ...
            *ti_gp_buff = ti_parent_blockno;
            ti_gp_buff++;
            (*no_blks_reqd)++;
            printf("TIPB %ld: %u\n", ti_parent_blockno, *total_remaining_blocks);
            ...
            for (j = 0; j < (fs->blksz / sizeof(int)); j++)
            {
                ...
                *ti_parent_buff = ti_child_blockno;
                ...
                for (k = 0; k < (fs->blksz / sizeof(int)); k++)
                {
                    actual_block_no = ext4fs_get_new_blk_no();
                    if (actual_block_no == -1)
                    {
                ...
                put_ext4(((uint64_t) ((uint64_t)ti_child_blockno *(uint64_t)fs->blksz)), ti_cbuff_start_addr, fs->blksz);
                free(ti_cbuff_start_addr);
            ...
            put_ext4(((uint64_t) ((uint64_t)ti_parent_blockno * (uint64_t)fs->blksz)), ti_pbuff_start_addr, fs->blksz);
        ...
        put_ext4(((uint64_t) ((uint64_t)ti_gp_blockno * (uint64_t)fs->blksz)), ti_gp_buff_start_addr, fs->blksz);
				...
				unsigned int *total_no_of_block)
{
    ...
    alloc_single_indirect_block(file_inode, &total_remaining_blocks, &no_blks_reqd);
    alloc_double_indirect_block(file_inode, &total_remaining_blocks, &no_blks_reqd);
    alloc_triple_indirect_block(file_inode, &total_remaining_blocks, &no_blks_reqd);
    *total_no_of_block += no_blks_reqd;
}

static struct ext4_extent_header *ext4fs_get_extent_block(struct ext2_data *data, char *buf,
                                                          ...
                                                          int log2_blksz)
{
    ...
    while (1)
    {
        ...
        if (le16_to_uint16(&(ext_block->eh_magic)) != EXT4_EXT_MAGIC)
            return 0;
        ...
        {
            i++;
            if (i >= le16_to_uint16(&(ext_block->eh_entries)))
                break;
        } while (fileblock >= le32_to_uint32(&(index[i].ei_block)));

        if (--i < 0)
            return 0;

        block = le16_to_uint16(&(index[i].ei_leaf_hi));
        block = (block << 32) + le32_to_uint32(&(index[i].ei_leaf_lo));
...
static int ext4fs_blockgroup(struct ext2_data *data, int group, struct ext2_block_group *blkgrp)
{
    long int blkno;
...
//    printf("ext4fs read %d group descriptor (blkno %ld blkoff %u)\n", group, blkno, blkoff);
    ...
    return ext4fs_devread((lbaint_t)blkno << (LOG2_BLOCK_SIZE(data) - log2blksz), blkoff, sizeof(struct ext2_block_group), (char *)blkgrp);
}
...
long int read_allocated_block(struct ext2_inode *inode, int fileblock)
{
    ...
    if (le32_to_uint32(&(inode->flags)) & EXT4_EXTENTS_FL)
    {
        ...
        int i = -1;
        ext_block = ext4fs_get_extent_block(ext4fs_root, buf, (struct ext4_extent_header *)inode->b.blocks.dir_blocks, fileblock, log2_blksz);
        if (!ext_block)
        {
        ...
        do
        {
            ...
            if (i >= le16_to_uint16(&(ext_block->eh_entries)))
                    break;
        } while (fileblock >= le32_to_uint32(&(extent[i].ee_block)));
        if (--i >= 0)
        {
            fileblock -= le32_to_uint32(&(extent[i].ee_block));
            if (fileblock >= le16_to_uint16(&(extent[i].ee_len)))
            {
                free(buf);
                return 0;
            }

            start = le16_to_uint16(&(extent[i].ee_start_hi));
            start = (start << 32) + le32_to_uint32(&(extent[i].ee_start_lo));
            free(buf);
    ...
    if (fileblock < INDIRECT_BLOCKS)
        blknr = le32_to_uint32(&(inode->b.blocks.dir_blocks[fileblock]));
    ...
    else if (fileblock < (INDIRECT_BLOCKS + (blksz / 4)))
    {
        ...
        if ((le32_to_uint32(&(inode->b.blocks.indir_block)) << log2_blksz) != ext4fs_indir1_blkno)
        {
            status = ext4fs_devread((lbaint_t)le32_to_uint32(&(inode->b.blocks.indir_block)) << log2_blksz, 0, blksz, (char *)ext4fs_indir1_block);
            if (status == 0)
            {
                printf("** SI ext2fs read block (indir 1) failed. **\n");
                return 0;
            }
            ext4fs_indir1_blkno = le32_to_uint32(&(inode->b.blocks.indir_block)) << log2_blksz;
        }
        blknr = le32_to_uint32(&(ext4fs_indir1_block[fileblock - INDIRECT_BLOCKS]));
    }
    /* Double indirect. */
    else if (fileblock < (INDIRECT_BLOCKS + (blksz / 4 * (blksz / 4 + 1))))
    {
        ...
        if ((le32_to_uint32(&(inode->b.blocks.double_indir_block)) << log2_blksz) != ext4fs_indir1_blkno)
        {
            status = ext4fs_devread((lbaint_t)le32_to_uint32(&(inode->b.blocks.double_indir_block)) << log2_blksz, 0, blksz, (char *)ext4fs_indir1_block);
            if (status == 0)
            {
                printf("** DI ext2fs read block (indir 2 1) failed. **\n");
                return -1;
            }
            ext4fs_indir1_blkno = le32_to_uint32(&(inode->b.blocks.double_indir_block)) << log2_blksz;
        }
        ...
        if ((le32_to_uint32(&(ext4fs_indir1_block[rblock / perblock])) << log2_blksz) != ext4fs_indir2_blkno)
        {
            status = ext4fs_devread((lbaint_t)le32_to_uint32(&(ext4fs_indir1_block[rblock / perblock])) << log2_blksz, 0, blksz, (char *)ext4fs_indir2_block);
            if (status == 0)
            {
                printf("** DI ext2fs read block (indir 2 2) failed. **\n");
                return -1;
            }
            ext4fs_indir2_blkno = le32_to_uint32(&(ext4fs_indir1_block[rblock / perblock])) << log2_blksz;
        }
        blknr = le32_to_uint32(&(ext4fs_indir2_block[rblock % perblock]));
    }
    /* Tripple indirect. */
    else
    {
        ...
        if ((le32_to_uint32(&(inode->b.blocks.triple_indir_block)) << log2_blksz) != ext4fs_indir1_blkno)
        {
            status = ext4fs_devread((lbaint_t)le32_to_uint32(&(inode->b.blocks.triple_indir_block)) << log2_blksz, 0, blksz, (char *)ext4fs_indir1_block);
            if (status == 0)
            {
                printf("** TI ext2fs read block (indir 2 1) failed. **\n");
                return -1;
            }
            ext4fs_indir1_blkno = le32_to_uint32(&(inode->b.blocks.triple_indir_block)) << log2_blksz;
        }
        ...
        if ((le32_to_uint32(&(ext4fs_indir1_block[rblock / perblock_parent])) << log2_blksz) != ext4fs_indir2_blkno)
        {
            status = ext4fs_devread((lbaint_t)le32_to_uint32(&(ext4fs_indir1_block[rblock / perblock_parent])) << log2_blksz, 0, blksz, (char *)ext4fs_indir2_block);
            if (status == 0)
            {
                printf("** TI ext2fs read block (indir 2 2) failed. **\n");
                return -1;
            }
            ext4fs_indir2_blkno = le32_to_uint32(&(ext4fs_indir1_block[rblock / perblock_parent])) << log2_blksz;
        }
        ...
        if ((le32_to_uint32(&(ext4fs_indir2_block[rblock / perblock_child])) << log2_blksz) != ext4fs_indir3_blkno)
        {
            status = ext4fs_devread((lbaint_t)le32_to_uint32(&(ext4fs_indir2_block[(rblock / perblock_child) % (blksz / 4)])) << log2_blksz, 0, blksz, (char *)ext4fs_indir3_block);
            if (status == 0)
            {
                printf("** TI ext2fs read block (indir 2 2) failed. **\n");
                return -1;
            }
            ext4fs_indir3_blkno = le32_to_uint32(&(ext4fs_indir2_block[(rblock / perblock_child) % (blksz / 4)])) << log2_blksz;
        }

        blknr = le32_to_uint32(&(ext4fs_indir3_block[rblock % perblock_child]));
    }
    printf("read_allocated_block %ld\n", blknr);
...
//        printf("Iterate dir %s\n", name);
    ...
    /* Search the file.  */
    while (fpos < le32_to_uint32(&(diro->inode.size)))
    {
        struct ext2_dirent dirent;

        status = ext4fs_read_file(diro, fpos, sizeof(struct ext2_dirent), (char *)&dirent, &actread);
        if (status < 0)
            return 0;
        ...
        if (dirent.namelen != 0)
        {
            ...
            status = ext4fs_read_file(diro, fpos + sizeof(struct ext2_dirent), dirent.namelen, filename, &actread);
            if (status < 0)
                return 0;
            ...
            fdiro->ino = le32_to_uint32(&(dirent.inode));
            ...
            {
                status = ext4fs_read_inode(diro->data, le32_to_uint32(&(dirent.inode)), &fdiro->inode);
                if (status == 0)
                {
                    free(fdiro);
                    return 0;
                ...
                if ((le16_to_uint16(&(fdiro->inode.mode)) & FILETYPE_INO_MASK) == FILETYPE_INO_DIRECTORY)
                {
                    type = FILETYPE_DIRECTORY;
                }
                else if ((le16_to_uint16(&(fdiro->inode.mode)) & FILETYPE_INO_MASK) == FILETYPE_INO_SYMLINK)
                {
                    type = FILETYPE_SYMLINK;
                }
                else if ((le16_to_uint16(&(fdiro->inode.mode)) & FILETYPE_INO_MASK) == FILETYPE_INO_REG)
                {
                    type = FILETYPE_REG;
                }
            }
...
//            printf("iterate >%s<\n", filename);
            ...
            else
            {
                ...
                printf("%10u %s\n", le32_to_uint32(&(fdiro->inode.size)), filename);
            }
            free(fdiro);
        }
        fpos += le16_to_uint16(&(dirent.direntlen));
    }
    return 0;
}
...
static char *ext4fs_read_symlink(struct ext2fs_node *node)
{
    ...
    symlink = malloc(le32_to_uint32(&(diro->inode.size)) + 1);
    if (!symlink)
        return 0;

    if (le32_to_uint32(&(diro->inode.size)) <= 60)
    {
        strncpy(symlink, diro->inode.b.symlink, le32_to_uint32(&(diro->inode.size)));
    }
    else
    {
        status = ext4fs_read_file(diro, 0, le32_to_uint32(&(diro->inode.size)), symlink, &actread);
    ...
    symlink[le32_to_uint32(&(diro->inode.size))] = '\0';
    return symlink;
}
			     ...
			     struct ext2fs_node **currfound, int *foundtype)
{
    ...
    for (;;)
    {
        ...
        if (type == FILETYPE_SYMLINK)
        {
            ...
            status = ext4fs_find_file1(symlink, oldnode, &currnode, &type);
...
int ext4fs_open(const char *filename, loff_t *len)
{
    ...
    ext4fs_file = NULL;
    status = ext4fs_find_file(filename, &ext4fs_root->diropen, &fdiro, FILETYPE_REG);
    if (status == 0)
        goto fail;

    if (!fdiro->inode_read)
    {
        status = ext4fs_read_inode(fdiro->data, fdiro->ino, &fdiro->inode);
        if (status == 0)
            goto fail;
    }
    *len = le32_to_uint32(&(fdiro->inode.size));
    ext4fs_file = fdiro;
