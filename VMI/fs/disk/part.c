...
static void print_part_header(const char *type, block_dev_desc_t *dev_desc)
{
    ...
    printf (" device  --   Partition Type: %s\n\n", type);
}
...
void print_part(block_dev_desc_t* dev_desc)
{
    ...
    {
        case PART_TYPE_DOS:
            printf("## %s valid DOS partition ##\n", dev_desc->devPathName); 
		       ...
		       disk_partition_t *info)
{
    ...
    switch (dev_desc->part_type)
    {
        case PART_TYPE_DOS:
            ...
            {
                printf("## Invalid DOS partition found ##\n");
            }
            break;
...
#error Only one of CONFIG_SANDBOX and CONFIG_CMD_UBIFS may be selected
	...
	if (!part_str || !*part_str) {
		...
		part = (int)strtoul(part_str, &ep, 16);
		/*
