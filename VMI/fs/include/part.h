...
#define DEV_PATH_NAME_MAX_SIZE (128)

struct block_dev_desc {
	int		if_type;	/* type of the interface */
	int		dev;		/* device number */
        char            devPathName[DEV_PATH_NAME_MAX_SIZE];
	...
	uint64_t	(*block_read)(block_dev_desc_t *block_dev, uint64_t lba, uint64_t cnt, uint8_t* buf);
...
static inline uint32_t le32_to_uint32(unsigned char *le32)
{
    return ((le32[3] << 24) + (le32[2] << 16) + (le32[1] << 8) + le32[0]);
}

/* Convert char[2] in little endian format to the host format integer */
static inline uint16_t le16_to_uint16(unsigned char *le16)
{
